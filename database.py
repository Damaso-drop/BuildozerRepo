import json
import os
import hashlib
import hmac
import base64
import tempfile
from uuid import uuid4
from datetime import datetime

try:
    from kivy.utils import platform
except ImportError:
    platform = 'linux'


def _get_data_dir():
    """Return a private, per-installation storage directory.

    On Android this is the app's internal 'files' directory. The OS
    guarantees this directory is empty on every fresh install and wipes
    it on uninstall — it is NOT part of the APK's bundled assets.
    """
    if platform == 'android':
        try:
            from android.storage import app_storage_path
            return app_storage_path()
        except Exception:
            pass
    return os.path.dirname(os.path.abspath(__file__))


# ---------------------------------------------------------------------------
# Password hashing (PBKDF2-HMAC-SHA256)
# ---------------------------------------------------------------------------
_PBKDF2_ITERATIONS = 200_000


def hash_password(password, salt=None):
    """Return (salt_b64, hash_b64) for the given plaintext password."""
    if salt is None:
        salt = os.urandom(16)
    elif isinstance(salt, str):
        salt = base64.b64decode(salt)
    dk = hashlib.pbkdf2_hmac(
        'sha256', (password or '').encode('utf-8'), salt, _PBKDF2_ITERATIONS)
    return base64.b64encode(salt).decode(), base64.b64encode(dk).decode()


def verify_password(password, salt_b64, hash_b64):
    try:
        salt = base64.b64decode(salt_b64)
        expected = base64.b64decode(hash_b64)
        dk = hashlib.pbkdf2_hmac(
            'sha256', (password or '').encode('utf-8'), salt, _PBKDF2_ITERATIONS)
        return hmac.compare_digest(dk, expected)
    except Exception:
        return False


# ---------------------------------------------------------------------------
# Table name normalization (case + whitespace insensitive)
# ---------------------------------------------------------------------------
def normalize_table_name(name):
    if not name:
        return ''
    return ' '.join(str(name).strip().split()).lower()


class Database:
    def __init__(self):
        self.filename = os.path.join(_get_data_dir(), 'barpos_data.json')
        self.data = self._load_data()
        self._init_defaults()
        self._migrate()

    # ------------------------------------------------------------------ io
    def _load_data(self):
        if not os.path.exists(self.filename):
            return {}
        try:
            with open(self.filename, 'r') as f:
                return json.load(f)
        except Exception:
            return {}

    def _save_data(self):
        """Atomic save: write to a temp file in the same directory,
        fsync, then os.replace(). A crash mid-write leaves the original
        file untouched."""
        directory = os.path.dirname(self.filename) or '.'
        try:
            fd, tmp_path = tempfile.mkstemp(
                prefix='.barpos_', suffix='.json', dir=directory)
            try:
                with os.fdopen(fd, 'w') as f:
                    json.dump(self.data, f, indent=4, ensure_ascii=False)
                    f.flush()
                    os.fsync(f.fileno())
                os.replace(tmp_path, self.filename)
            except Exception:
                try:
                    os.unlink(tmp_path)
                except OSError:
                    pass
                raise
        except Exception as e:
            print(f"[Database] atomic save failed ({e}); falling back")
            with open(self.filename, 'w') as f:
                json.dump(self.data, f, indent=4, ensure_ascii=False)

    # ------------------------------------------------------------------ migration
    def _migrate(self):
        changed = False

        for p in self.data.get('products', []):
            if 'priceTiers' not in p:
                p['priceTiers'] = []
                changed = True

        if 'debts' not in self.data:
            self.data['debts'] = []
            changed = True
        if 'license_data' not in self.data:
            self.data['license_data'] = {}
            changed = True

        # Password migration: plaintext -> pbkdf2
        for u in self.data.get('users', []):
            if 'password' in u and 'password_hash' not in u:
                salt_b64, hash_b64 = hash_password(u['password'])
                u['salt'] = salt_b64
                u['password_hash'] = hash_b64
                del u['password']
                changed = True

        # Case-insensitive table name dedupe
        seen_sessions = {}
        new_sessions = []
        for s in self.data.get('sessions', []):
            key = normalize_table_name(s.get('tableName', ''))
            if key in seen_sessions:
                base = seen_sessions[key]
                base.setdefault('orders', []).extend(s.get('orders', []))
                base['credit'] = base.get('credit', 0) + s.get('credit', 0)
                changed = True
            else:
                seen_sessions[key] = s
                new_sessions.append(s)
        if new_sessions != self.data.get('sessions', []):
            self.data['sessions'] = new_sessions

        if changed:
            self._save_data()

    def _init_defaults(self):
        changed = False

        if 'machine_id' not in self.data:
            self.data['machine_id'] = str(uuid4())[:16]
            changed = True

        if 'users' not in self.data:
            salt_b64, hash_b64 = hash_password('123')
            self.data['users'] = [{
                'username': 'admin',
                'salt': salt_b64,
                'password_hash': hash_b64,
                'role': 'admin'
            }]
            changed = True

        if 'categories' not in self.data:
            self.data['categories'] = []
            changed = True
        if 'products' not in self.data:
            self.data['products'] = []
            changed = True
        if 'sessions' not in self.data:
            self.data['sessions'] = []
            changed = True
        if 'transactions' not in self.data:
            self.data['transactions'] = []
            changed = True
        if 'debts' not in self.data:
            self.data['debts'] = []
            changed = True
        if 'license_data' not in self.data:
            self.data['license_data'] = {}
            changed = True

        if changed:
            self._save_data()

    # ------------------------------------------------------------------ license
    def get_machine_id(self):
        return self.data.get('machine_id', '')

    def set_machine_id(self, machine_id):
        self.data['machine_id'] = machine_id
        self._save_data()

    def get_license_key(self):
        return self.data.get('license_data', {}).get('key')

    def set_license_key(self, key):
        self.data.setdefault('license_data', {})['key'] = key
        self._save_data()

    def get_license_last_seen(self):
        return self.data.get('license_data', {}).get('last_seen')

    def update_license_last_seen(self, timestamp):
        self.data.setdefault('license_data', {})['last_seen'] = timestamp
        self._save_data()

    # ------------------------------------------------------------------ auth
    def authenticate(self, username, password):
        for u in self.data.get('users', []):
            if u['username'] != username:
                continue
            if 'password_hash' in u and 'salt' in u:
                if verify_password(password, u['salt'], u['password_hash']):
                    return u
                continue
            # Legacy plaintext (unmigrated) — accept once and hash
            if u.get('password') == password:
                salt_b64, hash_b64 = hash_password(password)
                u['salt'] = salt_b64
                u['password_hash'] = hash_b64
                u.pop('password', None)
                self._save_data()
                return u
        return None

    def verify_password(self, username, password):
        return self.authenticate(username, password) is not None

    def verify_user_password(self, user, password):
        """Check a password against a user dict already in memory."""
        if not user:
            return False
        if 'password_hash' in user and 'salt' in user:
            return verify_password(password, user['salt'], user['password_hash'])
        return user.get('password') == password

    # ------------------------------------------------------------------ users
    def username_exists(self, username):
        return any(u['username'] == username for u in self.data.get('users', []))

    def add_user(self, username, password, role='employee'):
        username = (username or '').strip()
        password = password or ''
        if not username or not password:
            return False, "Preencha todos os campos"
        if self.username_exists(username):
            return False, "Nome de usuário já existe"
        if role == 'admin' and password == '123':
            return False, "Senha do administrador não pode ser a padrão (123)"
        salt_b64, hash_b64 = hash_password(password)
        self.data.setdefault('users', []).append({
            'username': username,
            'salt': salt_b64,
            'password_hash': hash_b64,
            'role': role
        })
        self._save_data()
        return True, "Usuário criado com sucesso"

    def remove_user(self, username):
        target = next((u for u in self.data.get('users', [])
                       if u['username'] == username), None)
        if not target:
            return False, "Usuário não encontrado"
        if target['role'] == 'admin':
            remaining_admins = [
                u for u in self.data['users']
                if u['role'] == 'admin' and u['username'] != username
            ]
            if not remaining_admins:
                return False, "É preciso manter pelo menos um administrador"
        self.data['users'] = [u for u in self.data['users']
                              if u['username'] != username]
        self._save_data()
        return True, "Usuário removido"

    # ------------------------------------------------------------------ catalog
    def get_categories(self):
        return self.data.get('categories', [])

    def get_products(self):
        return self.data.get('products', [])

    def update_stock(self, product_id, change):
        for p in self.data['products']:
            if p['id'] == product_id:
                if p['stock'] + change < 0:
                    return False
                p['stock'] += change
                self._save_data()
                return True
        return False

    def add_category(self, name):
        new_cat = {'id': str(uuid4()), 'name': name}
        self.data.setdefault('categories', []).append(new_cat)
        self._save_data()

    # ------------------------------------------------------------------ sessions
    def get_sessions(self):
        return self.data.get('sessions', [])

    def get_session(self, table_name):
        key = normalize_table_name(table_name)
        for s in self.data.get('sessions', []):
            if normalize_table_name(s.get('tableName', '')) == key:
                return s
        return None

    def session_owner(self, table_name):
        s = self.get_session(table_name)
        return s.get('userId') if s else None

    def save_session(self, session_data):
        key = normalize_table_name(session_data.get('tableName', ''))
        self.data['sessions'] = [
            s for s in self.data['sessions']
            if normalize_table_name(s.get('tableName', '')) != key
        ]
        self.data['sessions'].append(session_data)
        self._save_data()

    def remove_session(self, table_name):
        key = normalize_table_name(table_name)
        self.data['sessions'] = [
            s for s in self.data['sessions']
            if normalize_table_name(s.get('tableName', '')) != key
        ]
        self._save_data()

    # ------------------------------------------------------------------ checkout
    def checkout(self, table_name, total, items, paid=None, change=None, user_id=None):
        t = {
            'id': str(uuid4()),
            'tableName': table_name,
            'totalAmount': total,
            'timestamp': datetime.now().isoformat(),
            'items': items
        }
        if paid is not None:
            t['paid'] = paid
        if change is not None:
            t['change'] = change
        if user_id:
            t['userId'] = user_id
        self.data['transactions'].append(t)
        self.remove_session(table_name)
        self._save_data()

    # ------------------------------------------------------------------ debts
    def add_debt(self, debt):
        debt['id'] = str(uuid4())
        debt['timestamp'] = datetime.now().isoformat()
        self.data['debts'].append(debt)
        self._save_data()

    def get_debts(self):
        return self.data.get('debts', [])

    def remove_debt(self, debt_id, restore_stock=False):
        """Remove a debt. When restore_stock=True (used for mistake
        corrections), the items are returned to stock. When False (used
        after a successful payment), stock stays decremented because the
        goods already left the bar."""
        target = next((d for d in self.data['debts'] if d['id'] == debt_id), None)
        if target and restore_stock:
            for item in target.get('items', []):
                pid = item.get('productId')
                qty = item.get('quantity', 0)
                if not pid or qty <= 0:
                    continue
                for p in self.data.get('products', []):
                    if p['id'] == pid:
                        p['stock'] += qty
                        break
        self.data['debts'] = [d for d in self.data['debts'] if d['id'] != debt_id]
        self._save_data()


db = Database()