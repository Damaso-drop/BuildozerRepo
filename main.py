...
from database import db, normalize_table_name
...
class RoleToggleButton(ToggleButton):
...
class ComboSpinner(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.arrow_btn = Button(
            ...
            background_color=Theme.BG_INPUT, color=Theme.TEXT_GRAY, font_size=sp(16)
        )
...
class LoadingPopup(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        self.progress = ProgressBar(max=0)
...
class QtyPopup(ModalView):
    ...
    max_qty = NumericProperty(1)

    def open_for(self, product, cb, max_qty=None):
        self.product = product
        self.product_name = product['name']
        self.callback = cb
        available = max_qty if max_qty is not None else product['stock']
        self.max_qty = available
        self.stock_label = f"Disponível: {available}"
        self.qty = 1 if available >= 1 else 0
        self.open()

    def adjust(self, val):
        new_qty = self.qty + val
        if self.product:
            if 1 <= new_qty <= self.max_qty:
                self.qty = new_qty

    def confirm(self):
        if self.callback and self.product and self.qty > 0:
...
class UserManagementModal(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        Clock.schedule_once(self._inject_role_selector, 0)

    def _inject_role_selector(self, dt):
        existing = None
        try:
            existing = self.ids['role_spinner']
        except Exception:
            existing = None
        if existing is not None:
            self._role_selector = existing
            return
        try:
            anchor = None
            for wid in ('new_user_pass_confirm', 'new_user_pass', 'new_user_name'):
                try:
                    anchor = self.ids[wid]
            ...
            group_name = f"user_role_grp_{id(self)}"
            role_row = BoxLayout(
            ...
            role_lbl.bind(size=lambda inst, val: setattr(inst, 'text_size', val))
            role_row.add_widget(role_lbl)

            btn_box = BoxLayout(orientation='horizontal', size_hint_x=0.70, spacing=dp(6))
            self._emp_btn = RoleToggleButton(text='Funcionário', group=group_name, font_size=sp(12))
            self._adm_btn = RoleToggleButton(text='Administrador', group=group_name, font_size=sp(12))
            btn_box.add_widget(self._emp_btn)
            btn_box.add_widget(self._adm_btn)
            role_row.add_widget(btn_box)
            parent.add_widget(role_row)
            self._emp_btn.state = 'down'
        except Exception as e:
            print(f"[UserManagementModal] role selector injection failed: {e}")

    def get_role(self):
        if self._role_selector is not None:
            try:
                return 'admin' if self._role_selector.text == 'Administrador' else 'employee'
            except Exception:
                return 'employee'
        if self._emp_btn is not None and self._emp_btn.state == 'down':
            return 'employee'
        if self._adm_btn is not None and self._adm_btn.state == 'down':
            return 'admin'
        return 'employee'
    ...
    def reset_role(self):
        if self._role_selector is not None:
            try:
                self._role_selector.text = 'Funcionário'
            except Exception:
                pass
            return
        if self._emp_btn is not None:
            self._emp_btn.state = 'down'
        if self._adm_btn is not None:
            self._adm_btn.state = 'normal'

    def reset_form(self):
        try:
            self.ids.new_user_name.text = ""
            self.ids.new_user_pass.text = ""
            self.ids.new_user_pass_confirm.text = ""
    ...
    def save(self, username, password, confirm):
        ...
        if not username or not password:
            ...
            return
        if password != confirm:
            self.ids.error_label.text = "As senhas não coincidem"
            return
        role = self.get_role()
        if role == 'admin' and password == '123':
            self.ids.error_label.text = "Senha do administrador não pode ser a padrão (123)"
            return
        if db.username_exists(username):
            self.ids.error_label.text = "Nome de usuário já existe"
            return
        self.ids.error_label.text = ""
        self.save_callback(username, password, role)
        self.reset_form()
        self.dismiss()
...
class ProductModal(ModalView):
    ...
    def refresh_tiers_list(self):
        ...
        for i, tier in enumerate(self.price_tiers):
            ...
            del_btn.add_widget(Label(text='X', color=Theme.TEXT_WHITE, bold=True, font_size=sp(12)))
    ...
    def load(self, debts):
        ...
        for d in debts:
            ...
            card = BoxLayout(orientation='vertical', size_hint_y=None,
                             height=dp(50 + 22*len(items) + 60), padding=dp(12), spacing=dp(6))
            ...
            lbl_items = Label(text=items_text.rstrip(), font_size=sp(14), color=Theme.TEXT_GRAY,
                              halign='left', valign='top', size_hint_y=None, height=dp(20*len(items) + 10))
            ...
            pay_btn = RoundedButton(text='Pagar', bg_color=Theme.PRIMARY, bold=True)
            pay_btn.bind(on_release=lambda x, did=d['id']: self.pay_callback(did))
            actions.add_widget(pay_btn)
            del_btn = RoundedButton(text='Excluir', bg_color=Theme.DANGER, bold=True)
            del_btn.bind(on_release=lambda x, did=d['id']: self.confirm_delete(did))
...
class ChangeConfirmationPopup(ModalView):
    ...
    def verify_password(self, pwd):
        ...
        if user and db.verify_user_password(user, pwd):
            self.callback()
            self.dismiss()
        else:
...
class AuthPopup(ModalView):
    ...
    def check(self, pwd):
        user = App.get_running_app().current_user
        if user and db.verify_user_password(user, pwd):
            self.callback(); self.dismiss()
        else:
            self.ids.error_lbl.text = "Senha Incorreta"
...
class TransactionsDetailsModal(ModalView):
    ...
    def update_data(self, trans):
        ...
        for t in trans:
            ...
            if 'userId' in t:
                box.add_widget(Label(text=f"Funcionário: {t['userId']}", size_hint_y=None, height=dp(20), font_size=sp(12), color=(0.7,0.9,0.7,1)))
...
class LicenseScreen(Screen):
    ...
    def on_enter(self):
        self.machine_id = db.get_machine_id()
        Clock.schedule_once(self.check_license)

    # ------------------------------------------------------------------
    # LAUNCH PATH — reads the encrypted grant, no raw key needed
    # ------------------------------------------------------------------
    def check_license(self, dt):
        try:
            machine_id = db.get_machine_id()

            # Auto-migrate: if we still have a plaintext key but no grant,
            # convert it now, then wipe the plaintext.
            if db.needs_grant_migration():
                try:
                    old_key = db.get_license_key()
                    blob = Lkm.create_grant(old_key, machine_id)
                    db.set_license_grant(blob)
                    print("[LicenseScreen] migrated plaintext key -> encrypted grant")
                except Exception as e:
                    print(f"[LicenseScreen] grant migration failed: {e}")

            blob = db.get_license_grant()
            if not blob:
                self.ids.error_label.text = "Nenhuma licença ativada"
                return

            grant = Lkm.read_grant(blob, machine_id)
            if not grant:
                self.ids.error_label.text = "Licença corrompida. Por favor reative."
                return

            last_seen = db.get_license_last_seen()
            tampered, new_last_seen = Lkm.check_time_tampering(last_seen)
            if tampered:
                self.ids.error_label.text = "Possível manipulação de relógio detectada."
                return

            valid, msg = Lkm.is_grant_valid(grant)
            if valid:
                db.update_license_last_seen(new_last_seen)
                Clock.schedule_once(
                    lambda dt: setattr(self.manager, 'current', 'login'), 0.1)
            else:
                self.ids.error_label.text = msg
        except Exception as e:
            self.ids.error_label.text = f"Erro: {e}"

    # ------------------------------------------------------------------
    # ACTIVATION PATH — runs ONCE, when the user pastes a raw key
    # ------------------------------------------------------------------
    def activate(self, key):
        try:
            key = (key or '').strip()
            if not key:
                self.ids.error_label.text = "Chave vazia"
                return

            machine_id = db.get_machine_id()

            valid, msg = Lkm.validate_license_key(key, machine_id)
            if not valid:
                self.ids.error_label.text = msg
                return

            blob = Lkm.create_grant(key, machine_id)
            db.set_license_grant(blob)      # raw key never touches the DB
            db.update_license_last_seen(time.time())
...
class POSScreen(Screen):
    ...
    license_time_left = StringProperty("")
    license_valid = BooleanProperty(True)

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._license_clock = None
        self._license_label = None
        self._license_row = None
    ...
    def on_enter(self):
        ...
        Clock.schedule_once(self._setup_license_row, 0.05)

    def on_leave(self):
        if self._license_clock is not None:
            self._license_clock.cancel()
            self._license_clock = None

    # ------------------------------------------------------------------
    # License row — inserted ONCE, guarded against duplicates
    # ------------------------------------------------------------------
    def _setup_license_row(self, dt=None):
        if self._license_row is not None:
            if self._license_row.parent is not None:
        ...
        row = BoxLayout(orientation='horizontal', size_hint=(1, None),
                        height=dp(26), padding=[dp(8), 0])
        with row.canvas.before:
            Color(*Theme.BG_CARD)
            bg_rect = Rectangle(pos=row.pos, size=row.size)
        def _upd_bg(*a):
            bg_rect.pos = row.pos
            bg_rect.size = row.size
        row.bind(pos=_upd_bg, size=_upd_bg)

        self._license_label = Label(text='', halign='center', valign='middle',
                                     color=Theme.TEXT_GRAY, font_size=sp(12), bold=True)
        self._license_label.bind(size=lambda i, v: setattr(i, 'text_size', v))
        row.add_widget(self._license_label)
        self._license_row = row
        ...
        for c in self.children:
            try:
                ...
                continue
        if root is None and self.children:
            root = self.children[-1]

        inserted = False
        if isinstance(root, BoxLayout) and root.orientation == 'vertical':
            try:
                existing = list(root.children)
                for w in existing:
                ...
                for w in reversed(existing):
                    root.add_widget(w)
                inserted = True
                print("[POSScreen] license row inserted at top of BoxLayout")
            except Exception as e:
                print(f"[POSScreen] BoxLayout rebuild failed: {e}")

        if not inserted:
            try:
                row.pos_hint = {'top': 1.0, 'x': 0}
                self.add_widget(row)
                print("[POSScreen] license row placed via pos_hint fallback")
            except Exception as e:
                print(f"[POSScreen] fallback failed: {e}")

        self._update_license_label()
        if self._license_clock is not None:
            self._license_clock.cancel()
        self._license_clock = Clock.schedule_interval(self._update_license_label, 30)

    def _update_license_label(self, dt=None):
        """Read the encrypted grant (no raw key, no validator)."""
        try:
            machine_id = db.get_machine_id()
            blob = db.get_license_grant()
            grant = Lkm.read_grant(blob, machine_id) if blob else None
            valid, msg = Lkm.is_grant_valid(grant)
            self.license_valid = valid
            self.license_time_left = f"Licença: {msg}"
        except Exception as e:
            self.license_valid = False
            self.license_time_left = f"Licença: erro ({e})"

        if self._license_label is not None:
            self._license_label.text = self.license_time_left
            self._license_label.color = Theme.TEXT_GRAY if self.license_valid else Theme.DANGER

    # ------------------------------------------------------------------
    # Tiered pricing helper (packs + remainder)
    # ------------------------------------------------------------------
    def _compute_tiered_price(self, unit_price, qty, tiers):
        if qty <= 0:
            return 0.0, 0, 0
        if not tiers:
            return unit_price * qty, 0, 0
        usable = [t for t in tiers
                  if t.get('quantity', 0) > 1 and t['quantity'] <= qty]
        if not usable:
            return unit_price * qty, 0, 0
        tier = max(usable, key=lambda t: t['quantity'])
        pack_qty = tier['quantity']
        pack_price = tier['totalPrice']
        packs = qty // pack_qty
        remainder = qty % pack_qty
        total = packs * pack_price + remainder * unit_price
        return total, packs, pack_qty
    ...
    def set_table(self, name):
        ...
        if existing and not self.is_admin:
            owner = existing.get('userId')
            if owner and user and owner != user['username']:
                self.info_popup.message = "Esta mesa pertence a outro funcionário."
                self.info_popup.open()
                return
        self.current_table = name
        if existing:
            self.cart = existing.get('orders', [])
            self.table_credit = existing.get('credit', 0)
        else:
            self.cart = []
            self.table_credit = 0
        self.calc_totals()
    ...
    def pay_debt(self, debt_id):
        ...
        for s in sessions:
            if normalize_table_name(s['tableName']) == normalize_table_name(debt.get('tableName', '')):
    ...
    def finalize_debt_payment(self, debt, paid, credit_used):
        ...
        db.checkout(debt['tableName'], debt['totalAmount'], debt['items'],
                    paid=total_paid, change=change, user_id=uid)
    ...
    def do_checkout_with_credit(self, paid):
        ...
        db.checkout(self.current_table, self.cart_total, self.cart,
                    paid=total_paid, change=change, user_id=uid)
        self.cart = []
        self.current_table = ""
        self.table_credit = 0
    ...
    def do_checkout(self, paid):
        ...
        db.checkout(self.current_table, self.cart_total, self.cart,
                    paid=paid, change=change, user_id=uid)
        self.cart = []
        self.current_table = ""
        self.table_credit = 0
        self.ids.table_input.text = ""
    ...
    def remove_debt(self, debt_id):
        ...
        if debt and (self.is_admin or debt.get('userId') == App.get_running_app().current_user['username']):
            db.remove_debt(debt_id, restore_stock=True)
            if self.debts_popup.parent:
                all_debts = db.get_debts()
                if not self.is_admin:
                    user = App.get_running_app().current_user
    ...
    def add_to_cart(self, product, qty):
        ...
        total, packs, pack_qty = self._compute_tiered_price(
            product['price'], total_qty, tiers)
        existing['total_price'] = total
        if packs > 0:
            existing['tier_quantity'] = pack_qty
    ...
    def delete_table_session(self, table_name):
        ...
        if session and (self.is_admin or session.get('userId') == user['username']):
            ...
            if normalize_table_name(self.current_table) == normalize_table_name(table_name):
                self.current_table = ""
                self.table_credit = 0
                self.ids.table_input.text = ""
                self.cart = []
    ...
    def _on_product_tap(self, product):
        ...
        if not fresh_product: return
        available = fresh_product['stock']
        if available <= 0:
        ...
        if not self.current_table:
            ...
            self.info_popup.open()
            return
        self.qty_modal.open_for(fresh_product, self.add_to_cart, max_qty=available)
    ...
    def save_session(self):
        if not self.current_table:
            return
        user = App.get_running_app().current_user
        existing = db.get_session(self.current_table)
        if existing and not self.is_admin:
            owner = existing.get('userId')
            if owner and user and owner != user['username']:
                self.info_popup.message = "Esta mesa pertence a outro funcionário."
...
class AdminScreen(Screen):
    ...
    def _calc_reports_thread(self):
        ...
        if self.report_filter == 'daily':
            ...
            filtered = [t for t in trans if t['timestamp'].startswith(target)]
        elif self.report_filter == 'myday':
            user = App.get_running_app().current_user
            uid = user['username'] if user else ''
            today = now.strftime('%Y-%m-%d')
            filtered = [t for t in trans
                        if t['timestamp'].startswith(today)
                        and t.get('userId') == uid]
    ...
    def open_user_modal(self):
        self.user_modal.reset_form()
        self.user_modal.open()
    ...
    def add_user(self, u, p, role='employee'):
        ok, msg = db.add_user(u, p, role)
        if not ok:
            try:
                self.user_modal.ids.error_label.text = msg
                if not self.user_modal.parent:
                    self.user_modal.open()
                return
            except Exception:
                info = InfoPopup(); info.message = msg; info.open()
                return
        self.refresh_data()
