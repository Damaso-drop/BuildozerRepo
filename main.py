...
class LicenseScreen(Screen):
    ...
    def check_license(self, dt):
        try:
            license_key = db.get_license_key()
            if not license_key:
                self.ids.error_label.text = "Nenhuma licença ativada"
                return
            last_seen = db.get_license_last_seen()
            tampered, new_last_seen = license_manager.check_time_tampering(last_seen)
            if tampered:
                self.ids.error_label.text = "Possível manipulação de relógio detectada."
                return
            valid, msg = license_manager.validate_license_key(license_key, db.get_machine_id())
            if valid:
    ...
    def activate(self, key):
        try:
            key = key.strip()
            valid, msg = license_manager.validate_license_key(key, db.get_machine_id())
            if valid:
                db.set_license_key(key)
...
class POSScreen(Screen):
    ...
    def on_enter(self):
        ...
        # Insert the license row after KV has done its initial layout pass
    ...
    # ------------------------------------------------------------------
    # SEPARATE ROW at the very top of the POS screen
    # ------------------------------------------------------------------
    def _setup_license_row(self, dt=None):
        """Insert a dedicated horizontal row at the top of the POS screen."""
        # Build the row
        ...
        # Find KV root: the full-size child of the screen
        root = None
        ...
        # Strategy A: root is a vertical BoxLayout → rebuild with row on top
        if isinstance(root, BoxLayout) and root.orientation == 'vertical':
            try:
                existing = list(root.children)   # reverse-add order
                for w in existing:
                    root.remove_widget(w)
                root.add_widget(row)             # row added first → sits on top
                for w in reversed(existing):     # restore original visual order
                    root.add_widget(w)
                inserted = True
        ...
        # Strategy B: place at absolute top of screen
        if not inserted:
            try:
                ...
                print(f"[POSScreen] fallback also failed: {e}")
    ...
    def _update_license_label(self, dt=None):
        try:
            key = db.get_license_key()
            mid = db.get_machine_id()
            valid, msg = license_manager.get_license_time_remaining(key, mid)
