...
class AdminScreen(Screen):
    ...
    def _do_delete_user(self, username):
        target = next((u for u in db.data['users'] if u['username'] == username), None)
        if not target:
            return
        remaining_admins = [u for u in db.data['users'] if u['role'] == 'admin' and u['username'] != username]
        if target['role'] == 'admin' and not remaining_admins:
            info = InfoPopup(); info.message = "Não é possível excluir: é preciso manter pelo menos um administrador."; info.open()
            return
